(function($) {
    ...
    $(document).ready(function() {
      ...
      if(current_page=='provider-verification'){
        $('.input-images-1').imageUploader({
            maxSize: 1 * 1024 * 1024,
            maxFiles: 3
        });
        ...
        $('#use_promo_code').on('click', function() {

           
            $('#AppliedCodeD').hide();
            $('#discount_check').val('Yes');
            var selected_promocodes = $('#selected_promocodes').val();
            ...
            $.ajax({
                ...
                data: {
                     selected_promocodes: selected_promocodes,
                ...
                success: function(response) {
                    if(response.status){
                        $("#use_promo_code").text("Discount Applied");
                        $("#use_promo_code"). attr("disabled", true);
                        $('#AppliedAmount').html(discount_amount);
                        $('#TotalAmount').html(amount_to_pay);
                        $('#AppliedCodeD').show();
                    }else{
        ...
        $('.update_pro_booking_status').on('click', function(e) {
          //  e.stopImmediatePropagation();
            ...
            update_pro_booking_status(id, status, rowid, review, provider);
        ...
        function update_pro_booking_status(bookid, status, rowid, category,provider) {
            $.confirm({
                title: 'Confirmations..!',
                ...
                buttons: {
                    confirm: function() {
                        $.ajax({
                            ...
                            data: { 'booking_id': bookid, 'status': status, 'provider': provider, 'csrf_token_name': csrf_token },
                            type: 'POST',
                            dataType: 'JSON',
                            beforeSend: function() {
    ...
    function delete_comments(id) {
        if(confirm("Are you sure you want to delete this Comment...!?")){
            $.post(base_url+'user/blogs/deleteComments',{comments_id:id,csrf_token_name:csrf_token},function(result){
                if(result) {
                    window.location.reload();
                }
            });
    ...
    $(document).on("click","#providerRefund",function(e){
        var id=$(this).attr('data-id');
      $('#providerRefundModal').modal('show');
    /*    $.ajax({
              type:'POST',
              url: base_url+'user/service/provider_refund_details',
              data : {id:id,csrf_token_name:csrf_token},
              dataType:'json',
              success:function(response)
              {
                $('#snId').html(response.subscription_name);
                $('#ssdId').html(response.subscription_date);
                $('#sedId').html(response.expiry_date_time);
                $('#smId').html(response.offer_amount);
                $('#baId').html(response.bookings_amount);
                $('#raId').html(response.offer_amount-response.bookings_amount);
                $('#pv_refund_amount').val(response.offer_amount-response.bookings_amount);
    ...
    $(document).on("click","#providerSubscriptionContinue",function(e){
        var pv_refund_amount=0;
        var refund_reason='';
        var request_type='Subscription';
        $.ajax({
            type:'POST',
            url: base_url+'user/service/provider_refund_request',
            data : {request_type:request_type,pv_refund_amount:pv_refund_amount,refund_reason:refund_reason,csrf_token_name:csrf_token},
            success:function(response)
            {
                alert('Your subscription is on the process. The Admin Will be approval it.')
                location.reload();
            }
       });
    });
    $(document).on("click","#refund_to_provider",function(e){

     var pv_refund_amount=$('#pv_refund_amount').val();
     var refund_reason=$('#refund_reason').val();
     var request_type='Refund';
     $('#refund_reason_required').hide();
     var flag=true;

     if(refund_reason==''){
        $('#refund_reason_required').show();
     }else{

        $('#refund_to_provider').prop("disabled", true);
        ...
        $.ajax({
            ...
            success:function(response)
            {
                alert('Your refund request is on the process. The Admin Will be approval it.')
                location.reload();
            }
       });
    }
    ...
    $(document).on("click",".provider_available",function(e){
		var id=$(this).attr('data-id');
        changeProviderAvailability(id);
	});
	function changeProviderAvailability(userId){
		var stat= $('#customSwitch1').prop('checked');
		if(stat==true) {
			var status=1;
		}
		else {
		...
		$.post(base_url+'user/service/change_provider_status',{id:userId,status:status,csrf_token_name:csrf_token},function(data){
