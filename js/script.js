(function($) {
  ...
  var sticky_header =$('#sticky_header').val();
	...
	if(current_page=="adv-post"){
		var delete_title = "Inactive ADV Post";
		var delete_msg = "Are you sure want to inactive this ADV Post?";
		var delete_text = "Your ADV Post has been Inactive";
	}
	if(current_page=="adv-post-inactive"){
		var delete_title = "Delete ADV Post";
		var delete_msg = "Are you sure want to delete this ADV Post?";
		var delete_text = "Your ADV Post has been Deleting";
	}
	$(document).on('click','.adv-inactive-post',function() {
		...
		$(document).on('click','.si_accept_confirm',function(){
			var url = base_url+'user/service/adv_inactive_post';
			$.ajax({
				url:url,
				...
				success: function(res){
					if(res==1) {
						window.location = base_url+'adv-post';
					}else if(res==2){
						window.location = base_url+'adv-post';
					}
				}
	...
	$(document).on('click','.delete-adv-post',function() {
		var s_id = $(this).attr("data-id");
		$('#deleteConfirmModal').modal('toggle');
		$('#acc_title').html('<i>'+delete_title+'</i>');
		$('#acc_msg').html(delete_msg);
		...
		$(document).on('click','.si_accept_confirm',function(){
			var url = base_url+'user/service/adv_delete_post';
			$.ajax({
				url:url,
				data:{s_id:s_id,csrf_token_name:csrf_token},
				type:"POST",
				...
				success: function(res){
					if(res==1) {
						window.location = base_url+'adv-post-inactive';
					}else if(res==2){
						window.location = base_url+'adv-post-inactive';
					}
				}
