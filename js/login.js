...
function check_emaillogin_otp(){
  ...
  if(mode==1){ //provider
   ...
   $.ajax({
    ...
    $.ajax({
      type: "POST",
      url: base_url+"user/login/checkemaillogin_bda",
      data:{  
        email:email,
        login_password:login_password,
          'csrf_token_name':csrf_token,
...
excluded: ':disabled',
 fields: {
   userName: {
     validators: {
      remote: {
        url: base_url + 'user/login/userName_chk',
...
referred_by: {
  validators: {
    remote: {
      url: base_url + 'user/login/refferal_chk',
      data: function(validator) {
        return {
          referred_by: validator.getFieldElements('referred_by').val(),
          csrf_token_name:csrf_token,
          checked: checked
        };
      },
    ...
    regexp: {
      regexp: '^[a-zA-Z0-9]*$',
      message: 'The value is not a valid refferal code'
    },
  }
},
userPassword: {
...
userMobile: {
 ...
 regexp: {
  ...
  var categorys=    $('#categoryname').val();
  ...
  var providercaptcha=   $('#provider_captcha').val();
  ...
  if(user_logintype=='email')
  {
   ...
   'category':categorys,
	'subcategory':subcategorys,
     ...
     'providercaptcha':providercaptcha,
     'is_agree':agree,
     'csrf_token_name':csrf_token,
     'mobileno':userMobile
   },
   ...
   success: function (data) { 
     ...
     if(obj.response=='ok')
     { 
		   window.location.href = base_url+"provider-verification";
     }
     else
     {
...
$('#forgetpwddiv').bootstrapValidator({
  ...
  $('#new_third_page_user').bootstrapValidator({
    ...
    var user_captchas =   $('#user_captcha').val();
...
/* form validation for bda registration starts */
$('#bda_new_third_page_user').bootstrapValidator({
  excluded: ':disabled',
  fields: {
    userName: {
      validators: {
  ...
  var user_captchas =   $('#bda_user_captcha').val();
  var userName      =   $('#bda_user_name').val();
  var userEmail     =   $('#bda_user_email').val();
  var userPassword  =   $('#bda_user_password').val();
  var userMobile    =   $('#bda_user_mobile').val();
  var countryCode   =   $("#bda_user_mobile").intlTelInput("getSelectedCountryData").dialCode;
  var user_logintype=   $('#bda_user_logintype').val();
  var agree         =   $('#bda_agree_checkbox_user').val();

  if(user_logintype=='email') {
    $.ajax({
      type: "POST",
      url: base_url+"user/login/emailregisterbda",
      data:{ 
        'name':userName,
        'email':userEmail,
        'password':userPassword,
      ...
      success: function (data) { 
        ...
        if(obj.response=='ok') { 
          ...
          $('#bda_registration_submit_user').prop('disabled',false).text('Register');
        }
      }
    }); 
  }
...
/* form validation for bda registration ends */
